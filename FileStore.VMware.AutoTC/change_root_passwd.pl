...
my $ip = $ARGV[0];
my $root_password = $ARGV[1];

#Add Keys to authorised keys in B
$password = "symantec";
$timeout = 30;
my $aft = new Expect;
$aft->spawn("ssh -o \"UserKnownHostsFile=/dev/null\" -o \"StrictHostKeyChecking no\" support\@$ip") or die "Cannot ssh to the machine \n";
$aft->expect($timeout,[ qr'\? $',     sub { my $fh=shift;$fh->send("yes\n"); exp_continue; } ],
    	              [ 'Password: $',sub { my $fh=shift;$fh->send("$password\n");exp_continue;}  ],  
		       '-re','\# $'	
		  ) or die "Cannot authenticate";
$aft->send("passwd root\n");
$aft->expect($timeout,[ qr'\? $',     sub { my $fh=shift;$fh->send("yes\n"); exp_continue; } ],
    	              [ 'assword: $',sub { my $fh=shift;$fh->send("$root_password\n");exp_continue;}  ],  
		       '-re','\# $'	
		  ) or die "Cannot change password";
$aft->do_soft_close();
